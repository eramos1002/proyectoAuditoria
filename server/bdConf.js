...
const bdInit = () => {
    console.log(`[MONGOOSE]: Contectango con base de datos MongoDB con la URL: ${mongoUrl}...`);

    // Debug de las operciones
    mongoose.set('debug', (collectionName, method, query, options, doc) => {
        console.log("[MONGOOSE]: " + collectionName + "." + method + "(" + JSON.stringify(query) + ") " + JSON.stringify(options));
    });
    ...
    .catch(err => console.error("[MONGOOSE]: Error al conectar con la Base de Datos", err));
};
